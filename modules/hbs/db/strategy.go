...
func QueryStrategies(tpls map[int]*model.Template) (map[int]*model.Strategy, error) {
	...
	sql := fmt.Sprintf(
		...
		"s.id, s.metric, s.tags, s.func, s.op, s.right_value, s.max_step, s.priority, s.note, s.tpl_id, s.alarm_recovery",
		now,
		now,
		now,
		now,
	...
	for rows.Next() {
		...
		var tags string
		var tid int
		err = rows.Scan(&s.Id, &s.Metric, &tags, &s.Func, &s.Operator, &s.RightValue, &s.MaxStep, &s.Priority, &s.Note, &tid, &s.AlarmRecovery)
		if err != nil {
			log.Println("ERROR:", err)
			continue
		}
